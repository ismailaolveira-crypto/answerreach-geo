...
function Test-VolumeDatabase {
    ...
    & docker run --rm --mount "type=volume,src=$DataVolume,dst=/source,readonly" alpine:3.21 test -f /source/geo_platform.db *> $null
    return $LASTEXITCODE -eq 0
}
...
function New-PersonalConfig {
    ...
    $content = "GEO_AUTH_SECRET=$secret`nGEO_HTTP_PORT=$port`nGEO_WORKER_CONCURRENCY=8`nGEO_WORKER_INSTANCE_ID=$workerInstanceId`n"
...
function Backup-Data([bool]$RestoreOnSuccess = $true) {
    ...
    try {
        ...
        & docker run --rm --mount "type=volume,src=$DataVolume,dst=/source,readonly" --mount "type=bind,src=$backupDir,dst=/backup" alpine:3.21 sh -c "test -f /source/geo_platform.db && cd /source && tar -czf /backup/data.tar.gz ."
        if ($LASTEXITCODE -ne 0) { throw "数据库备份失败。" }
        & docker run --rm --mount "type=volume,src=$ArtifactVolume,dst=/source,readonly" --mount "type=bind,src=$backupDir,dst=/backup" alpine:3.21 sh -c "cd /source && tar -czf /backup/artifacts.tar.gz ."
        if ($LASTEXITCODE -ne 0) { throw "证据备份失败。" }
        & docker run --rm --mount "type=bind,src=$backupDir,dst=/backup,readonly" alpine:3.21 sh -c "tar -tzf /backup/data.tar.gz >/dev/null && tar -tzf /backup/artifacts.tar.gz >/dev/null"
    ...
    Write-Host "备份已保存到：$backupDir"
    Write-Host "备份包含本机密钥和可能的 Provider 配置，请勿上传或转发。"
}
