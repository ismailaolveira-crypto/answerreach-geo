...
> 版本：1.2 · 更新：2026-08-09 · 状态：已启用的执行规约
...
1. 「GEO 机会判断 Agent」：只在用户选定批次并点击后，阅读该范围的完整真实证据，决定是否存在值得执行的优先机会；
2. 「GEO 内容研究与起草 Agent」：只在用户选中已持久化的 Codex 机会后，把它转换为有出处、可审核、符合目标平台规则的内容草稿。

两类任务都不是自动发布机器人。机会判断不写稿，内容起草不重新发明机会。
...
- 在用户未启动机会判断前生成、预填或展示任何新机会；
- 从指定批次和证据 ID 之外虚构一条“优先机会”；
...
机会判断任务使用 `schema_version=geo-opportunity-analysis.v1`，必须包含工作区、一个已完成批次、可选模型/问题范围、`input_fingerprint` 和通过真实性门禁的证据数组。输出只能是：

- `opportunities[]`：问题、优先级、目标信源/平台、缺失内容、竞品内容规律、理由、信心、不确定项和精确证据 ID；
- `no_action_reasons[]`：当证据不支持行动时的明确理由。

后端必须二次校验问题、证据 ID、来源 URL 和平台范围。Codex 运行成功但输出为空时，界面显示“未发现可靠机会”，不用规则卡片补位。

内容起草任务继续使用以下 `geo-agent-task.v1`：
...
同步助手请求成功只能标记 `request_accepted`；浏览器桥接返回草稿 URL 时只标记 `draft_link_returned`。用户打开该地址并明确确认正文可见，或受信任的后端适配器完成真实页面回读后，才可标记 `draft_saved`。
...
同一工作区允许最多 10 个活动 Agent/机会判断任务。队列 worker 和本机 Codex 客户端池也以 10 为硬上限；第 11 个任务必须返回真实容量冲突，不得悄悄排成“运行中”。每个 turn 使用独立的常驻客户端租约，一个租约失败只回收自己的客户端，不得重置其他在途任务。
