...
本设计提出时，「优化行动」仍是人工填写表单。当前实现已将它升级为真实证据驱动的行动工作台，并持续围绕三个问题验收：

1. 现在最值得解决哪个缺口；
2. 应该补哪一种真实信源；
3. 做完以后有没有在下一批次变好。

当前 v1 已支持：从明确批次、模型和问题范围发现并持久化机会；调用本机 Codex 生成可核验内容；人工审核后交给文章同步助手写入平台草稿；人工发布后归档公开 URL；最后创建同问题、同模型、同重复次数的复测并比较变化。系统不会把连接测试、等待队列、草稿写入或人工发布前状态记成完成。
