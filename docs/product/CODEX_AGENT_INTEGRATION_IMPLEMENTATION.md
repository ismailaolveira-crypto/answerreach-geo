...
行动页不单独维护一套可以手动推进的“假进度”。文章行动的页面状态必须由内容稿件、平台适配稿、同步任务、草稿回读、公开发布和核验证据共同推导。同一目标同时有多条记录时，以下表中更靠上、证据更强的状态为准。

| 页面状态 | 必须存在的真实记录 | 页面允许的操作 |
|---|---|---|
| `核验完成` | 符合平台域名的公开 URL，且公开页核验通过 | 查看证据、发起局部复测 |
| `等待发布` | 用户已打开草稿并确认正文可见，草稿 URL 已回读 | 打开已确认草稿，发布后填入公开 URL |
| `草稿待确认` | 助手已返回符合平台域名的候选草稿 URL | 打开草稿，由人确认正文是否真实可见 |
| `写入中` | 草稿写入请求已入库，尚未返回可用草稿链接 | 查看逐平台结果，不允许人工假装写入成功 |
| `可写入草稿` | 目标平台的适配稿已审核通过 | 打开 GEO 文章助手 |
| `待审核` | 已生成内容或平台适配稿，但没有审核通过 | 进入实际稿件审核页 |
| `生成中` | 存在正在运行的 Agent 任务或正在归档的内容资产 | 查看真实运行进度 |
| `待准备` | 尚无该平台可用的内容资产 | 开始准备内容 |

硬性规则：

- GEO 文章助手“已连接”只代表扩展可用，不代表各平台已登录；
- 每个平台的登录、草稿、发布和核验状态独立计算，一个平台成功不得带动其他平台；
- 候选草稿链接不等于草稿回读成功，草稿回读成功不等于已发布；
- 行动表中的状态是上述证据的投影和缓存，读取时必须重新校准，不能成为唯一真值来源；
- 所有跳过真实稿件、草稿回读或公开 URL 验证的手动推进请求，后端统一拒绝。

## 7. 设置页应该怎么改

内容生成不再要求用户填写模型 API Key。本机 Codex 与草稿交付分成两个能力状态；文章同步助手的本机路径和 Token 只保留在设置页的独立交付区，并始终脱敏回显。
...
优先行动工作台的“Codex 执行”设置从本机 runtime 实时读取模型目录，不在前端绑定某一个模型。用户先选模型，再从该模型实际支持的 reasoning effort 中选择推理强度。这两个值同时用于机会发现、官网分析和内容 Agent，并写入现有任务 JSON 快照，不增加数据库列。

后端必须在入队前校验“模型存在且推理强度受支持”。请求未显式传值时，使用 runtime 当前默认模型及其默认推理强度，以保持旧客户端兼容。观测筛选中的“模型”仍表示被分析的历史回答范围，与“Codex 执行模型”分开，不得混用。

### 文章同步助手

显示 MCP Server 路径、Token 是否已配置和只读连接测试。Token 不回显原值；账号登录态只在用户点击「打开同步助手」后由扩展返回，不做后台定时探测。

### 品牌事实库

设置页在 Agent 与草稿交付之前提供独立事实底座。每条事实包含名称、可公开陈述、公开来源 URL 和启用状态；停用不删除记录，也不改写历史草稿。官网正文无法从原始 HTML 回读时，至少一条启用且带公开来源的事实是启动官网 Agent 的前置条件。
...
实施状态（2026-08-09）：公开 URL 人工归档、同问题/同模型/同重复次数复测、证据差值和四类结论已经落地。Agent 默认实施工作区级 `10` 个活动 run 容量、15 分钟单次超时、SDK `interrupt`、排队取消竞态收敛和原 thread 恢复；队列 worker 与本机 Codex 常驻连接池均以 10 为上限，客户端按真实任务按需创建，单个客户端故障不会关闭其他在途 turn。页面直接显示容量、时限和可恢复操作。最终发布仍由人工在平台侧完成。跨机器分布式调度不属于当前单机产品范围。
...
官网事实门禁现已前后端一致：只有审计发现服务端正文缺失/过短或必须依赖客户端渲染时，才强制要求带公开来源的品牌事实。Agent 结果保存实际使用的事实 ID 和数量；没有可追溯品牌事实的历史结果显示为整改框架，审批 API 与界面按钮都拒绝把它批准为官网成稿。
