...
- 春秋元泉真实官网验收中，首页 HTML 为 JavaScript 外壳；系统在同域 `DownloadDialog-*.js` 中返回 10 条候选，并将“所有 AI 调用都通过「春秋元泉」实现统一接入……”再次通过严格核验后保存为事实 `#3`。本地现在有 1 条可用于新生成的事实；其余 2 条仍保持待核验，不伪装完成。
