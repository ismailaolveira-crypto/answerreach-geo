...
- Agent 只能提议官网截图候选，不能宣称已采集。真实截图通过质量门禁后，后端会把“建议截取/未执行截图”等事前占位语转为事后真实用途；前端也会对已归档的旧 manifest 做同样的显示校正，避免同一卡片同时出现“真实截图”与“未执行截图”。
...
- 优先行动顶部的稿件摘要与内容库使用同一事实快照口径：当事实库已有可追溯事实，而某个待处理稿件引用数为 0 时，必须计入“需重新生成”，不能继续冒充“待审核”。
- 审核包进一步区分“活跃但未完成原文核验”与“已完成原文核验”的品牌事实，并返回稿件匹配的未核验事实数。“有来源 URL”不再被等同于“事实已核验”；行动摘要会把这类稿件计入需重新生成，内容库显示独立的“事实待核验”状态，审核与交付继续被阻止。
- 官网行动的前端启动门禁与后端共用严格口径：只有 `source_verification.status=source_and_statement_verified` 的活跃事实才计入可用数量；仅填来源 URL 不会让“启动 Agent”提前显示可用。
- 审核警告会直达设置页 `#brand-facts` 事实区。待核验记录可按一次启动串行核验，但每条仍单独调用后端公网与原文核验并写入独立审计记录；界面显示当前事实、完成数、通过/失败数，部分失败不回滚成功结果，也不会提前冒充完成。
- 失败核验会以 `workspace.brand_fact.source_verification_failed` 写入审计日志，只保存来源 URL、陈述 SHA-256、HTTP 状态和公开错误原因，不保存密钥。最新一次失败会撤销同一事实的旧验证证明，并在设置页刷新后继续显示原因与时间。
- 待核验事实可调用 `POST /api/v1/workspaces/{workspaceId}/brand-facts/{factId}/source-candidates`：后端复用公网/内网隔离、重定向校验和固定字节预算，从服务端可见正文或最多 8 个同域公开 JavaScript 资源中筛选自然中文候选。第三方脚本、代码噪声、登录/下载等非事实文案不会进入结果。
- 同一官网的候选会按当前事实名称和旧陈述的字符片段相似度重新排序；这是确定性排序，不调用模型，也不会生成官网不存在的新句子。
- 候选只在对应事实行下方自然展开，显示“官网可见正文”或“同域公开前端资源”及精确证据 URL；点击只填入编辑器，仍需人工点击“核验并保存”。候选发现以 `workspace.brand_fact.source_candidates_discovered` 持久化来源、哈希、候选和检查时间，但不会自动改写事实或解除审核门禁。
- 历史稿件不能只凭品牌事实 ID 继承后来修改的证明：审核包同时要求 Claim 文本和来源 URL 与当前已核验事实完全一致。事实原文变化后，旧 Claim 会立即失去匹配并要求重新生成。
- Agent 生成后、保存资产前还有一道品牌原文硬门禁：任何以已核验品牌事实来源为 URL 的 `source_linked` Claim，其文本必须逐字匹配当前事实。在原文前补品牌名、近义改写、拆分或合并都会让本轮生成失败，不再降级冒充普通公开来源。
- 春秋元泉真实官网验收中，首页 HTML 为 JavaScript 外壳；系统在同域 `DownloadDialog-*.js` 中返回 10 条候选，三条品牌事实均经人工选择原文并通过严格同域核验。`Run #2` 的第三轮修订已生成内容 `#4 / v3`：三条品牌 Claim 逐字匹配当前事实与官网来源，另有五条 Google 官方规则来源，三条内部官网审计判断保持 `pending`。运行状态为 `awaiting_review`，未写平台草稿、未同步、未发布。
