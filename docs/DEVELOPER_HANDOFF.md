...
- 常规草稿交付以 EgoLite 当前页面注入的 `$syncer` 为准，不依赖设置页保存的 MCP Server 路径。设置页“检测当前页面”会真实调用 `getAccounts`，只把知乎/微信公众号账号计为可写入平台；`zip-download` 等工具能力不能冒充第二个平台。后台 MCP 路径和 Token 仅保留在折叠的高级诊断区，已配置不等于当前浏览器可交付。
