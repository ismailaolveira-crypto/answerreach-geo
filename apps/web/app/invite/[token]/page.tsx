...
export default async function InvitePage({
	...
	return <main className="cq-auth-page">
		...
		<section className="cq-auth-panel-wrap"><div className="cq-auth-card"><div className="cq-auth-card-heading"><div className="cq-auth-panel-brand"><img alt="" aria-hidden="true" src="/brand/answerreach-mark.svg" /><span><b>入答</b><small>AnswerReach</small></span></div><h2>{invitation ? `加入 ${invitation.workspace_name}` : "邀请不可用"}</h2><p>{invitation ? `${invitation.email_hint} 将以“${roleLabels[invitation.role]}”身份加入。已有账号请填写原密码；新成员将直接创建本地工作账号。` : "这个邀请可能已过期、被撤销或已经使用。请联系工作区管理员重新生成。"}</p></div>
