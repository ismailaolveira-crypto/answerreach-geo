...
export function CompetitorInsightReport({
  ...
  const scope = { workspaceId, periodDays, modelKey, questionPlanId };
  ...
  if (!loaded) return <main className={styles.page}><div className={styles.loading}>正在读取本次会话报告…</div></main>;

  if (!insight) return <main className={styles.page}>
    <section className={styles.empty}>
      <span>◇</span><p>竞品范围分析报告</p><h1>尚未生成这个筛选范围的报告</h1>
      <div>报告不会伪造或补写。请先回到竞品对比，按相同筛选范围生成分析。</div>
    ...
    <div className={styles.reportShell}>
      ...
      <article className={styles.report}>
        <header className={styles.reportHeader}>
          ...
          <dl><div><dt>分析范围</dt><dd>{insight.scope.period}</dd></div><div><dt>真实回答</dt><dd>{insight.scope.answer_count} 条</dd></div><div><dt>生成模型</dt><dd>{insight.provider} · {insight.model}</dd></div></dl>
        </header>
        ...
        <footer className={styles.footer}>由 {insight.provider} · {insight.model} 于 {generatedAt} 基于当前筛选范围生成。{insight.analysis.limitations.join(" ")}</footer>
      </article>
    </div>
  </main>;
}
