...
export function WorkspaceSettingsForm({
  ...
  return <form action={action} className={styles.form}>
    ...
    <label>品牌名称<input name="brand_name" defaultValue={workspace.brand_name} required maxLength={255} /></label>
    <label>品牌别名<textarea name="brand_aliases" defaultValue={workspace.brand_aliases.join("\n")} placeholder="每行一个别名，例如：春秋元泉、元泉" maxLength={1000} /></label>
    <label>官网或主域名<input name="website_url" type="url" defaultValue={workspace.website_url ?? ""} placeholder="https://example.com" maxLength={500} /></label>
    ...
    <button type="submit" disabled={pending}>{pending ? "正在保存…" : "保存品牌设置"}</button>
  </form>;
}
